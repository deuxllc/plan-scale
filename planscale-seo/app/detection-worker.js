...
function collectRuns(binary, width, height, horizontal, minRun) {
  ...
  for (let primary = 0; primary < primarySize; primary++) {
    ...
    while (secondary < secondarySize) {
      ...
      if (end - start + 1 >= minRun) runs.push({ start, end, axis: primary });
    }
  }
  return runs;
}
...
function mergeRuns(runs, axisTolerance) {
  ...
  for (const run of runs) {
    ...
    if (!bestGroup) {
      groups.push({ minAxis: run.axis, maxAxis: run.axis, starts: [run.start], ends: [run.end] });
      continue;
    }
...
function groupsToSegments(groups, horizontal, minRun, maxThickness, processScale) {
  ...
  for (const group of groups) {
    ...
    if (length < minRun || thickness > maxThickness || length / Math.max(1, thickness) < 4) continue;
    segments.push(horizontal
      ? { start: { x: start / processScale, y: axis / processScale }, end: { x: end / processScale, y: axis / processScale } }
      : { start: { x: axis / processScale, y: start / processScale }, end: { x: axis / processScale, y: end / processScale } });
  }
  return segments;
}
...
function areSimilarSegments(a, b) {
  ...
  if (aHorizontal) {
    ...
    const overlap = rangesOverlap(Math.min(a.start.x, a.end.x), Math.max(a.start.x, a.end.x), Math.min(b.start.x, b.end.x), Math.max(b.start.x, b.end.x));
  ...
  const overlap = rangesOverlap(Math.min(a.start.y, a.end.y), Math.max(a.start.y, a.end.y), Math.min(b.start.y, b.end.y), Math.max(b.start.y, b.end.y));
  return overlap / Math.min(segmentLength(a), segmentLength(b)) > 0.85;
}
